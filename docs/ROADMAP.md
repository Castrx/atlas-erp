...
### Sprint 6A — Fundação de testes automatizados
Primeira cobertura real de testes do projeto, nos dois lados, em duas frentes sequenciais. Objetivo: fundação reutilizável + cobertura dos fluxos críticos (autenticação, Products, Customers, Dashboard) — não cobertura total do sistema. Nenhuma funcionalidade nova, nenhuma alteração de regra de negócio, nenhuma alteração de código de produção no backend (`src/main` intocado).

- **Backend** (`atlas-backend/src/test/`): Postgres efêmero via Testcontainers (mesma imagem do `docker-compose.yml`), completamente isolado do banco de desenvolvimento — `mvn test` não exige mais `docker compose up -d postgres` manual antes, só o Docker em execução. `AbstractIntegrationTest` (container único por JVM, cada teste numa transação revertida ao final) e `TestDataFactory` (dados únicos via UUID, sem colisão entre testes) como infraestrutura reutilizável. 27 testes unitários (Mockito puro) cobrindo `JwtService`, `AuthenticationService`, `ProductService`, `CustomerService` e `DashboardService` — este último focado nas duas áreas que já tiveram bugs reais na Sprint 3A (faturamento nulo, preenchimento de dias sem venda). 19 testes de integração (MockMvc + Testcontainers, ponta a ponta real) cobrindo `AuthController`, `ProductController` e `CustomerController`, incluindo autenticação JWT genuína e as regras de negócio de duplicidade (SKU, documento, e-mail). Total: 47 testes.
- **Frontend** (`atlas-frontend/src/`): Vitest + React Testing Library, configurado sobre a infraestrutura já existente (`vite.config.ts`), sem depender de nenhuma ferramenta nova além do necessário. `src/test/setup.ts` (jest-dom, cleanup automático entre testes, stubs de `scrollIntoView`/`matchMedia`/`ResizeObserver` que o MUI exige em ambiente jsdom) e `src/test/test-utils.tsx` (`render` customizado com `QueryClientProvider` + `MemoryRouter`, `createQueryWrapper` para testes de hook) como infraestrutura reutilizável. 13 testes cobrindo `ProtectedRoute` (redireciona vs. renderiza, conforme autenticação), `ProductFormDialog` e `CustomerFormDialog` (validação client-side, payload correto ao service mockado, mensagem de erro do backend repassada) e `useProducts`/`useCustomers` (estados de sucesso e erro do TanStack Query). Os services (`*.service.ts`) são mockados no lugar dos hooks — o teste continua exercitando TanStack Query e React Hook Form + Zod reais.
